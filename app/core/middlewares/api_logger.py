...
from app.core.utils.slack_handlers import SlackHandler
...
async def api_logger(request: Request, response=None, error=None):
    ...
    # TODO: token 통해서 미들웨어에서 유저 정보 가져와서 유저정보 로그 찍히도록 수정
    """
    ...
    """
    user_log = dict(client=request.state.ip)
    ...
    if error:
        ...
        slackHandler.send_message(data)
    else:
